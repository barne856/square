...
namespace lit {
// TODO: fix commented functions. May have index orders flipped
template <typename T>
...
class transform {
  public:
    ...
    void face_towards(squint::fvec3 point, squint::fvec3 up) {
        ...
        transformation_matrix = squint::inv(view);
        // transformation_matrix = squint::inv(view) * get_scale_matrix();
    }
    void translate(squint::fvec3 offset) {
        transformation_matrix[0][3] += offset[0];
        transformation_matrix[1][3] += offset[1];
        transformation_matrix[2][3] += offset[2];
    }
